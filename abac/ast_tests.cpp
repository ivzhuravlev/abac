...
TEST_CASE("OperatorNode", "[node]")
{
	...
	shared_ptr<Node> sum = make_shared<OperatorNode>(OperatorType::Plus,
		right1, right2);

	shared_ptr<Node> prod = make_shared<OperatorNode>(OperatorType::Prod,
		left, sum);

	shared_ptr<Node> divZero = make_shared<OperatorNode>(OperatorType::Div,
		prod, make_shared<OperandNode>());
